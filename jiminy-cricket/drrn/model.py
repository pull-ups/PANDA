...
def convert(a, persona_reverse):
    #False여야 q-value를 지키고, True면 깎는 것
    tup=()
    for i in range(len(a)):
        temp_a=a[i].clone()
        for j in range(len(temp_a)):
            # if not persona_reverse:
            #     if int(temp_a[j].tolist())<0:
            #         temp_a[j]=True
            #     else:
            #         temp_a[j]=False
            if not persona_reverse:
...
class DRRN(torch.nn.Module):
    ...
    def forward(self, state_batch, act_batch, poss_acts, detach=False, cond_weight=0,
        ...
        if cclm:
            if cclm.persona:
                cond_probs_persona=cclm.get_probs(state_batch, poss_acts, cond_threshold=cond_threshold, testing_flag=testing_flag)
                #print("in model.py, cond_probs_persona: ", cond_probs_persona)
                cond_probs=convert(cond_probs_persona, cclm.persona_reverse)
                #print("in model.py, cond_probs: ", cond_probs)
            else:
            ...
            if cond_weight >= 0:
                if args.adaptive_cond_weight:
                    ...
                    #print("in model.py, q_values_before: ", q_values)
                    q_values = [vals - cond_weight * probs for vals, probs in zip(q_values, cond_probs)]
                    #print("in model.py, q_values_after: ", q_values)
            else:
        ...
        #print("after q_values: ", q_values)
        # Split up the q-values by batch
        if cclm :
            return q_values, cond_probs_persona
        else:
    ...
    def act(self, states, poss_acts_tokenized, poss_acts, lm=None, eps=None, alpha=0, k=-1, argmax=False, cond_weight=0, cclm=None, cond_threshold=0, args=None):
        ...
        #print("in agent, poss_acts: ", poss_acts)
        #print("in agent, q_values: ", q_values)
        #print("in agent, cond_probs: ", cond_probs)
        ...
        if eps is None:  # sample ~ softmax(act_values)
            ...
            #print("before sampling, act_values: ", act_values)
            act_idxs = [sampling_func(vals).item() for vals in act_values]
            #print("sampled index: ", act_idxs)
            #print("vals: ", act_values)
