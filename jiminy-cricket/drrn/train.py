...
def train(agent, lm, envs, max_steps, update_freq, eval_freq, checkpoint_freq, log_freq, args, cclm):
	...
	for step in range(1, max_steps + 1):
		...
		for i, env in enumerate(envs): 
			#print("<observation> ", obs[i].strip().replace("\n", " "), "</observation>")
			observation_arr.append(obs[i].strip().replace("\n", " "))

		observations_arr.append({step:observation_arr})
		# act
		...
		print("act_strings: ", act_strings)
		print("action_idxs: ", action_idxs)
		for i in range(8):
			print(f"env {i} action list: ", infos[i]['valid'])
		...
		for i, env in enumerate(envs): 
			action_anns.append(
				[(cond_probs[i][j].tolist(), infos[i]['valid'][j]) for j in range(len(infos[i]['valid']))]
				)
			# print('State  {}: {}'.format(step, lm.tokenizer.decode(states[i].state)))
			# print('Actions{}: {}'.format(step, infos[i]['valid']))
			# print('Qvalues{}: {}'.format(step, [round(value, 2) for value in action_values[i].tolist()]))
			# print('CondProbs{}: {}'.format(step, [round(value, 2) for value in cond_probs[i].tolist()]))
		# step with rejection
		#print("action_strs:", action_strs)
		#print("action_anns:", action_anns)
		...
		for i, (env, action) in enumerate(zip(envs, action_strs)):
			if dones[i]:
				...
				#print(f"<log> env:  {i} game over </log>")
				#log_arr.append({f"step: {step} log_{i}": "game over"})
				log_arr.append("game over")				
				continue
			prev_inv, prev_look = infos[i]['inv'], infos[i]['look']
			#ob, reward, done, info = env.step(action, write_to_log=True)
			ob, reward, done, info = env.step(action, write_to_log=False)
			...
			if args.reward_shaping:
				...
				if args.persona_reverse:
					reward = reward - args.condition_weight * cclm_reward
				else:
					reward = reward + args.condition_weight * cclm_reward
			# ========================== #
			action_and_results=[[] for _ in range(args.num_envs)]
			action_and_results[i].append([step, action, reward, info['score'], done, markers, label_vectors])
			...
			if args.lm_top_k:  # deal with rejection
				...
				while not done and rej and len(actions) > 1:
					...
					rej = reject(ob) and prev_inv == info['inv'] and prev_look == info['look']
					#print("rej, checkpoint1 : ", rej)
				action_strs[i] = action
				
				for v, a in action_anns[i]:
					if a == action:
						annotation=v
						if not annotation:	
							cumulative_persona_cnt+=1
						break

				print("<log>",
				"env: ", i,
				"step: ", action_and_results[i][-1][0],
				"action: ", action_and_results[i][-1][1],
				"annotation: ", annotation,
				"reward: ", action_and_results[i][-1][2],
				"score: ", action_and_results[i][-1][3],
				"done: ", action_and_results[i][-1][4],
				"markers: ", action_and_results[i][-1][5],
				"label_vectors: ", action_and_results[i][-1][6],
				"</log>"
				)
				log_str=f"action: {action_and_results[i][-1][1]} / annotation: {annotation} / reward: {action_and_results[i][-1][2]} / score: {action_and_results[i][-1][3]}"
				#log_arr.append({f"step: {step} log_{i}": log_str})
				log_arr.append(log_str)
				
    
				information_str=f"env {str(i)} valid actions: "
				for _ in range(len(infos[i]['valid'])):
					information_str+=f"{infos[i]['valid'][_]} / {cond_probs[i][_]}, "
				information_arr.append(information_str)
		...
		logs_arr.append({step:log_arr})
		informations_arr.append({step:information_arr})
		...
		if args.lm_top_k:
			...
			#print("in lm.batched_generate")
			#print("batched_states:", batched_states)
		...
		if step % log_freq == 0:
			...
			tb.dumpkvs()
			str_temp=f'Step: {env_steps}\
			 ETHICS: {cumulative_morality_vector} \
			 PERSONA : {cumulative_persona_cnt} \
			 FPS: {round((step * args.num_envs) / (time.time() - start), 2)} \
			 Max score seen: {max_score} \
			 #dict: {len(lm.generate_dict)} \
			 Last50EpisodeScores: {sum(env.get_end_scores(last=50) for env in envs) / len(envs)}'
			print(str_temp)
			result_arr.append({f"step: {step} result": str_temp})
			
			save_json(f"{args.output_dir}/observation.json", observations_arr)
			save_json(f"{args.output_dir}/log.json", logs_arr)
			save_json(f"{args.output_dir}/result.json", result_arr)
			save_json(f"{args.output_dir}/generated_actions.json", informations_arr)
		...
		print("step time: ", step_endtime-step_starttime)
...
def parse_args():
	...
	parser.add_argument('--model_parallel', action='store_false')
	...
	# PERSONA
	parser.add_argument('--persona', default='Psychopathy', type=str)
	parser.add_argument('--persona_reverse', action='store_true')
...
def main():
	...
	if args.cclm_path:
		...
		cclm = EthicsModel(args.cclm_model, args.cclm_path,
							args.conditioning_model_load_dir or args.load_dir, persona=args.persona, persona_reverse=args.persona_reverse,  oracle=args.cclm_oracle)
	else:
		cclm = None
