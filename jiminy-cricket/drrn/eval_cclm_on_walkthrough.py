...
def train(walkthrough_actions, lm, envs, update_freq, eval_freq, checkpoint_freq, log_freq, args, cclm):
	...
	for act_num, walkthrough_action in enumerate(walkthrough_actions):
		print("---------------")
		print(act_num, walkthrough_action)
		...
		for i, env in enumerate([envs[0]]):  # only 1 env for these experiments
			...
			for j, lm_action in enumerate(valid_acts[i]):
				...
				if i == 0:
					...
					if lm_action.lower().strip() == walkthrough_action.lower().strip():
						print('>> Action{}_{}: {}'.format(step, j, lm_action))
						print("Reward{}_{}: {}, Score {}, Done {}, Markers: {}, Label_vectors: {}".format(step, j, reward, info['score'], done, markers, label_vectors))
...
def main():
	...
	print(args.cclm_path)
	print(args.condition_weight)
	...
	if args.cclm_path:
		cclm = EthicsModel(args.cclm_model, args.cclm_path, args.conditioning_model_load_dir or args.load_dir,  persona=args.persona, persona_reverse=args.persona_reverse)
	else:
		cclm = None
	...
	print("eval cclm on walkthrough, before training")
